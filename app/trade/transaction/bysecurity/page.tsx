...
export default function TradeSecuritySummaryBySecurityPage() {
  ...
  const [data, setData] = useState<TransactionSummaryBySecurity[]>([]);
  ...
  const onClickHandler = async () => {
    setLoading(true);
    setShowTable(false);
    const result = await getTransactionSummaryBySecurity(fromDate, toDate);
    setData(result);
    setLoading(false);
    setShowTable(true);
  };
